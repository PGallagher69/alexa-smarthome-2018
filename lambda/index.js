...
const DeviceControl_Handler =  {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        return request.type === 'IntentRequest' && request.intent.name === 'DeviceControl' ;
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        ...
        let say = 'Hello from DeviceControl. ';
        ...
        //   SLOT: device 
        if (slotValues.device.heardAs) {
            slotStatus += ' slot device was heard as ' + slotValues.device.heardAs + '. ';
        } else {
            slotStatus += 'slot device is empty. ';
        }
        if (slotValues.device.ERstatus === 'ER_SUCCESS_MATCH') {
            
            deviceResolved = slotValues.device.resolved;

            slotStatus += 'a valid ';
            if(slotValues.device.resolved !== slotValues.device.heardAs) {
                slotStatus += 'synonym for ' + slotValues.device.resolved + '. '; 
                } else {
                slotStatus += 'match. '
            } // else {
                //
        }
        if (slotValues.device.ERstatus === 'ER_SUCCESS_NO_MATCH') {
            slotStatus += 'which did not match any slot value. ';
            console.log('***** consider adding "' + slotValues.device.heardAs + '" to the custom slot type used by slot device! '); 
        }

        if( (slotValues.device.ERstatus === 'ER_SUCCESS_NO_MATCH') ||  (!slotValues.device.heardAs) ) {
            slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('DeviceControl','device'), 'or');
        }
        //   SLOT: state 
        ...
        if (slotValues.state.ERstatus === 'ER_SUCCESS_MATCH') {
            ...
            stateResolved = slotValues.state.resolved;
        ...
        if( (slotValues.state.ERstatus === 'ER_SUCCESS_NO_MATCH') ||  (!slotValues.state.heardAs) ) {
            slotStatus += 'A few valid values are, ' + sayArray(getExampleSlotValues('DeviceControl','state'), 'or');
        }

        say += slotStatus;

        return new Promise(resolve => {
            updateShadow(deviceResolved, stateResolved, (status) => {
                let speechOutput = "";
                if (status) {
                    speechOutput = status;
                } else {
                    speechOutput = "Couldn't control the gadget";
                }
...
function updateShadow(device, desiredState, callback) {
    ...
    iotData.updateThingShadow(paramsUpdate, function(err, data) {
      if (err) {
        ...
        console.log(
          "updated thing shadow " +
            config.IOT_THING_NAME +
            " to state " +
            paramsUpdate.payload
        );
